clear all
clc
lg = {imread("hg_02_350.bmp"), imread("hg_11.bmp"), imread("hg_21_125.bmp"), imread("hg_33.png")};
% Parâmetros comuns
...
% Definindo os modos
modes = [2, 0; 1, 1; 2, 1; 3, 3];
num_modes = size(modes, 1);

% Plotando gráficos para cada tipo de feixe
plot_graphs('Hermite-Gaussian Beams', @hermite_gauss_phase_mask, modes, E0, h, delta, x, y, N, lg);
...
function plot_graphs(title_text, Phase_func, modes, E0, h, delta, x, y, N, expri)
    ...
    for idx = 1:num_modes
        ...
        Phase = Phase_func([N, N], mode1, mode2, 'scale',29.5, "range", [0 pi]);

        % Aplicação da máscara de fase
        E_masked = E0 .* exp(1i * Phase);

        % Propagação do feixe
        E_propagated = conv2D(E_masked, abs(E_masked), delta);
