...
function pattern = hermite_gauss_phase_mask(sz, xmode, ymode, varargin)
    ...
    pattern = (pattern - min(pattern(:))) / (max(pattern(:)) - min(pattern(:))); % Normalize to [0, 1]
    pattern = pattern * (range(2) - range(1)) + range(1); % Scale to specified range
